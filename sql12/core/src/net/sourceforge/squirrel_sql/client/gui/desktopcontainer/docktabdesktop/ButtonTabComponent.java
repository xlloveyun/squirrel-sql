...
public class ButtonTabComponent extends JPanel {
	...
	private Font _selectedFont = null; // the font of the title label if tab is
										// selected
	private Color _selectedColor = null; // the foreground color of the title
	...
	public ButtonTabComponent(IApplication app, JTabbedPane tabbedPane, String title, Icon icon) {
		...
		setOpaque(false);
		// the CloseTabButton needs a referrence to the ButtonTabComponent
		...
		_btnToWindow = new SmallTabButton(
				s_stringMgr.getString("docktabdesktop.ButtonTabComponent.detachButtonTooltip"),
				app.getResources().getIcon(SquirrelResources.IImageNames.TAB_DETACH_SMALL));

		_pnlSmallTabButtons.add(_btnToWindow);
	}

	// if look and feel changes we have to set the new defaults for rendering
	// the title label
	@Override
	...
	private void initLabelDefaults() {
		if (_label != null) {
			...
			// some look and feels may not support the above property so we fall
			// back to foreground color of tabbed pane
	...
	public void paint(Graphics g) {
		...
		if (tabIndex >= 0) {
			if (tabIndex == _tabbedPane.getSelectedIndex()) {
				...
				// check if the foreground color is not set by user through a
				// call to setForegroundAt
	...
	private static class CloseTabButton extends SmallTabButton {
		...
		protected void paintComponent(Graphics g) {
			...
			// If tab is selected we want to paint the x with selected
			// foreground color
